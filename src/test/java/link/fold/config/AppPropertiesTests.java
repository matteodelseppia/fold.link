...
/**
 * Covers valid, missing-required, and out-of-range binding for
 * {@link AppProperties}, independent of the full application context.
 */
class AppPropertiesTests {
    ...
    void usesAdrDefaultRetryCountWhenOmitted() {
        ...
        runner.withPropertyValues("app.base-url=https://fold.link", "app.alias.length=8")
                .run(context -> {
                    assertThat(context).hasNotFailed();
                    AppProperties props = context.getBean(AppProperties.class);
    ...
    void failsWhenBaseUrlIsNotAWellFormedUrl() {
        runner.withPropertyValues("app.base-url=not-a-url")
                .run(context -> assertThat(context).hasFailed());
    }

    @Test
    void failsWhenAliasLengthTooShort() {
        runner.withPropertyValues("app.base-url=https://fold.link", "app.alias.length=3")
                .run(context -> assertThat(context).hasFailed());
    }

    @Test
    void failsWhenAliasLengthTooLong() {
        runner.withPropertyValues("app.base-url=https://fold.link", "app.alias.length=64")
                .run(context -> assertThat(context).hasFailed());
    }

    @Test
    void failsWhenRetryCountIsNotPositive() {
        runner.withPropertyValues("app.base-url=https://fold.link", "app.alias.retry-count=0")
                .run(context -> assertThat(context).hasFailed());
    }

    @Test
    void failsWhenRetryCountTooHigh() {
        runner.withPropertyValues("app.base-url=https://fold.link", "app.alias.retry-count=21")
                .run(context -> assertThat(context).hasFailed());
    }
}
